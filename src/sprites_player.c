...
static void	move_player(t_game *game, int new_x, int new_y, int player_sprite)
{
	int	last_x;
	int last_y;
	...
	if (game->map->matrix[new_y][new_x] == EXIT && game->map->coins == 0)
	{
		ft_victory(game);
		return;
	}

	// Si el jugador se mueve a un piso o recoge una moneda
	if (game->map->matrix[new_y][new_x] == FLOOR || game->map->matrix[new_y][new_x] == COINS)
	{
		// Actualiza la posición anterior del jugador a FLOOR
		game->map->matrix[last_y][last_x] = FLOOR;

		// Si recoge una moneda, decrementa el contador
		if (game->map->matrix[new_y][new_x] == COINS)
			game->map->coins--;
